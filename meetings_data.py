...
class OddsFluctuation:
    ...
    def from_dict(cls, data: dict) -> "OddsFluctuation":
        if data is None:
            return cls(price=0.0, rolling_mean_deviation=0.0, time="")
        ...
        sydney_time = utc_time.replace(tzinfo=timezone.utc).astimezone(tz=None)
...
class Prediction:
    ...
    def from_dict(cls, data: dict) -> "Prediction":
        ...
        return cls(
            ...
            normalized_speed_position=data.get("normSpeedMeasureRatingName", ""),
    ...
    def from_dict(cls, data: dict) -> "Jockey":
        ...
        if len(last_year_places_list) < 3:
            ...
            if last_year_runs == 0 or last_year_runs is None:
                ...
                last_year_win_percentage = float(last_year_wins / last_year_runs)
                last_year_place_percentage = float(last_year_places / last_year_runs)
        return cls(
            id=data.get("id", 0),
            name=data.get("name", ""),
...
class FormBenchmark:
    ...
    def from_dict(cls, data: dict) -> "FormBenchmark":
        ...
        if runner_tempo_quantile_rank_data is None:
            runner_tempo_quantile_rank = None
        elif type(runner_tempo_quantile_rank_data) is str:
        ...
        if runner_time_difference_l800_data is None:
            runner_time_difference_l800 = None
        elif type(runner_time_difference_l800_data) is str:
        ...
        if runner_time_difference_l600_data is None:
            runner_time_difference_l600 = None
        elif type(runner_time_difference_l600_data) is str:
        ...
        if runner_time_difference_l400_data is None:
            runner_time_difference_l400 = None
        elif type(runner_time_difference_l400_data) is str:
        ...
        if runner_time_difference_l200_data is None:
            runner_time_difference_l200 = None
        elif type(runner_time_difference_l200_data) is str:
        ...
        if runner_race_position_l800_data is None:
            runner_race_position_l800 = 0
        elif type(runner_race_position_l800_data) is str:
        ...
        if runner_race_position_l600_data is None:
            runner_race_position_l600 = 0
        elif type(runner_race_position_l600_data) is str:
        ...
        if runner_race_position_l400_data is None:
            runner_race_position_l400 = 0
        elif type(runner_race_position_l400_data) is str:
        ...
        if runner_race_position_l200_data is None:
            runner_race_position_l200 = 0
        elif type(runner_race_position_l200_data) is str:
        ...
        if runner_meeting_position_l800_data is None:
            runner_meeting_position_l800 = 0
        elif type(runner_meeting_position_l800_data) is str:
        ...
        if runner_meeting_position_l600_data is None:
            runner_meeting_position_l600 = 0
        elif type(runner_meeting_position_l600_data) is str:
        ...
        if runner_meeting_position_l400_data is None:
            runner_meeting_position_l400 = 0
        elif type(runner_meeting_position_l400_data) is str:
        ...
        if runner_meeting_position_l200_data is None:
            runner_meeting_position_l200 = 0
        elif type(runner_meeting_position_l200_data) is str:
...
class PositionSummary:
    ...
    def from_dict(cls, data: dict) -> "PositionSummary":
        if data is None:
            return cls(distance=0, position=None, time="")
        return cls(
...
class Run:
    ...
    def from_dict(cls, data: dict) -> "Run":
        ...
        if track_condition_data is None or track_rating_data is None:
            track_condition = ""
        else:
        ...
        return cls(
            ...
            prize_money=data.get("racePrizeMoney", 0.0),
...
class PreparationStats:
    ...
    def from_runs(cls, runs: list[Run]) -> "PreparationStats":
        non_trial_runs: list[Run] = [run for run in runs if not run.is_trial]

        first_up_runs = [run for run in non_trial_runs if run.runs_since_spell == 0]
        second_up_runs = [run for run in non_trial_runs if run.runs_since_spell == 1]
        third_up_runs = [run for run in non_trial_runs if run.runs_since_spell == 2]
        nth_up_runs = [run for run in non_trial_runs if run.runs_since_spell == 3]
        ...
        if len(first_up_runs) != 0:
            first_up_win_percentage = float(
                len([run for run in first_up_runs if run.finish_position == 1])
                / len(first_up_runs)
            )
            ...
            if len(first_up_differences) > 0:
                first_up_average_difference = sum(first_up_differences) / len(
                    first_up_differences
                )
        ...
        if len(second_up_runs) != 0:
            second_up_win_percentage = float(
                len([run for run in second_up_runs if run.finish_position == 1])
                / len(second_up_runs)
            )
            ...
            if len(second_up_differences) > 0:
                second_up_average_difference = sum(second_up_differences) / len(
                    second_up_differences
                )
                if len(second_up_differences) >= 2:
                    second_up_stdev_difference = stdev(second_up_differences)
        ...
        if len(third_up_runs) != 0:
            third_up_win_percentage = float(
                len([run for run in third_up_runs if run.finish_position == 1])
                / len(third_up_runs)
            )
            ...
            if len(third_up_differences) > 0:
                third_up_average_difference = sum(third_up_differences) / len(
                    third_up_differences
                )
                if len(third_up_differences) >= 2:
                    third_up_stdev_difference = stdev(third_up_differences)
        ...
        if len(nth_up_runs) != 0:
            nth_up_win_percentage = float(
                len([run for run in nth_up_runs if run.finish_position == 1])
                / len(nth_up_runs)
            )
            nth_up_differences = []
            for run in nth_up_runs:
                if run.form_benchmark.runner_time_difference is not None:
                    nth_up_differences.append(run.form_benchmark.runner_time_difference)
            if len(nth_up_differences) > 0:
                nth_up_average_difference = sum(nth_up_differences) / len(
                    nth_up_differences
                )
    ...
    def get_preparation_stats(self, run_since_spell: int) -> tuple[float, float, float]:
        if run_since_spell == 0:
            return (
                self.first_up_win_percentage,
                self.first_up_average_difference,
                self.first_up_stdev_difference,
            )
        elif run_since_spell == 1:
            return (
                self.second_up_win_percentage,
                self.second_up_average_difference,
                self.second_up_stdev_difference,
            )
        elif run_since_spell == 2:
            return (
                self.third_up_win_percentage,
                self.third_up_average_difference,
                self.third_up_stdev_difference,
            )
        else:
            return (
                self.nth_up_win_percentage,
                self.nth_up_average_difference,
...
class Selection:
    ...
    def add_runs(self, runs: list[dict]):
        ...
        non_trial_runs: list[Run] = [run for run in self.runs if not run.is_trial]
    ...
    def add_stats(self, stats: dict):
        ...
        if wet_runs != 0:
            ...
            if len(wet_places) == 3:
                self.wet_runs_win_percentage = float(wet_places[0] / wet_runs) * 100
                wet_placings = wet_places[1] + wet_places[2]
                self.wet_runs_place_percentage = float(wet_placings / wet_runs) * 100
...
class Event:
    ...
    def from_dict(cls, data: dict) -> "Event":
        ...
        if track_condition_data is not None:
            ...
            if overall is None or rating is None:
                track_condition = ""
            else:
