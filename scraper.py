...
class MeetingsScraper:
    ...
    def create_meetings_time_query(self, scrape_date: datetime) -> QueryRequest:
        ...
        end_of_day = tomorrow.replace(hour=12, minute=59, second=59, microsecond=999)
        ...
        meetings_query = QueryInfo(
            query_type=QueryType.MEETINGS_TIME, variables=meetings_variables
        )
    ...
    def create_meetings_date_query(self, scrape_date: datetime) -> QueryRequest:
        ...
        meetings_query = QueryInfo(
            query_type=QueryType.MEETINGS_DATE, variables=meetings_variables
        )
        return QueryRequest(query_info=meetings_query)
    ...
    def create_meeting_query(self, slug: str) -> QueryRequest:
        ...
        meeting_query = QueryInfo(
            query_type=QueryType.MEETING_SLUG, variables=meeting_variables
        )
    ...
    def create_event_query(self, event_id: str) -> QueryRequest:
        ...
        event_query = QueryInfo(query_type=QueryType.EVENT, variables=event_variables)
    ...
    def create_stats_query(self, event_id: str) -> QueryRequest:
        ...
        stats_query = QueryInfo(query_type=QueryType.STATS, variables=stats_variables)
    ...
    def create_odds_query(self, event_id: str) -> QueryRequest:
        ...
        odds_query = QueryInfo(query_type=QueryType.ODDS, variables=odds_variables)
    ...
    def create_form_query(self, selection_ids: list) -> QueryRequest:
        form_query = QueryInfo(
            ...
            variables=FullFormQueryVariables(selectionIds=selection_ids, limit=10),
        )
    ...
    def parse_meeting_response(self, response: dict) -> Optional[Meeting]:
        ...
        for index, event in enumerate(meeting.events):
            ...
            if all(
                ...
                continue
            meeting.events[index].selections.sort(key=lambda x: x.number)
            ...
            for form in forms_data:
                for selection in meeting.events[index].selections:
                    if selection.id == form.get("selectionId"):
                        ...
                        selection.runs.sort(
                            key=lambda x: datetime.strptime(x.meeting_date, "%Y-%m-%d"),
                            reverse=True,
                        )
    ...
    def get_meetings(
        ...
        if time_query:
            ...
            meetings_slugs = self.parse_meetings_response_time(meetings_response)
        else:
            meetings_request = self.create_meetings_date_query(scrape_date)
            meetings_response = meetings_request.send_request()
            meetings_slugs = self.parse_meetings_response_date(meetings_response)
